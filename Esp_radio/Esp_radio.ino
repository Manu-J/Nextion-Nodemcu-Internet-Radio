...
Ticker           NextionBufferingUpdate;                    //For Nextion Screen Buffer Display
...
String			 icyGenre;								   // Genre from metadata
...
/**********************
 This timer will update the nextion display with buffer ratee every 500ms
 */
 
 void UpdateNextionBuffer()
 {
  SendNextionTextBoxCommand("page1.t5.txt=","Buff:"+String(rcount));
 }
...
void showstreamtitle ( const char *ml, bool full )
{
 ...
 SendNextionTextBoxCommand("page0.g0.txt=",streamtitle);
 SendNextionTextBoxCommand("page0.t0.txt=",icyname.c_str());
 SendNextionTextBoxCommand("page0.g1.txt=",icyGenre.c_str());
 SendNextionTextBoxCommand("page0.t2.txt=",String(bitrate)+"kb/s");
 SendNextionCommand("page0.h0.val="+String(vs1053player.getVolume()));
 SendNextionTextBoxCommand("page1.t6.txt=",String(pfs));
...
bool connectwifi()
{
  ...
  if (  WiFi.waitForConnectResult() != WL_CONNECTED )  // Try to connect
  {
    ...
    SendNextionTextBoxCommand("page2.t0.txt=","WiFi Failed!AP - " + String(NAME));
    SendNextionTextBoxCommand("page2.t1.txt=",String(pfs));
    return false ;
  }
  pfs = dbgprint ( "IP = %d.%d.%d.%d",
                   WiFi.localIP()[0], WiFi.localIP()[1], WiFi.localIP()[2], WiFi.localIP()[3] ) ;
...
void SendNextionTextBoxCommand(String CMD,String Val) { // Used to serially push out a String with Serial.write()
...
void SendNextionCommand(String CMD) { // Used to serially push out a String with Serial.write()
        Serial.write(0xff);
        Serial.write(0xff);
        Serial.write(0xff); 
        Serial.print(CMD);
    ...
    while ( inifile.available() )
    {
      ...
      if ( line.startsWith ( "preset_" ) )               // Found the key?
      {
        ...
        if ( inx > 0 )                                   // Hash sign present?
        {
          ...
          if ( inx > 0 )                                   
          {
            line.remove ( 0, inx + 2 ) ; // Yes, remove non-comment part
          }
        }
        else
        {
		...
		if(k<=50){
        stringThree ="page5.b"+ String(k);
        SendNextionTextBoxCommand(stringThree + ".txt=",line);
		    SendNextionCommand(stringThree+ ".txt_maxl=30");
		}
        k=k+1;
...
void setup()
{
  ...
  SendNextionTextBoxCommand("page2.t1.txt=","");
  SendNextionTextBoxCommand("page2.t0.txt=","Connected with Board");
  ...
  SendNextionTextBoxCommand("page2.t0.txt=","Checking SPIFF...");
  if ( fs_info.totalBytes == 0 )
  {
    dbgprint ( "No SPIFFS found!  See documentation." ) ;
    SendNextionTextBoxCommand("page2.t0.txt=","No SPIFFS found!...");
  }
  ...
  SendNextionTextBoxCommand("page2.t0.txt=","WIFI NETWORKS SRCH...");
  ...
  //SendNextionTextBoxCommand("page2.t0.txt=","ESP Version "+ VERSION);           
  dbgprint ( "Sketch size %d, free size %d",
             ESP.getSketchSize(),
             ESP.getFreeSketchSpace() ) ;
  pinMode ( BUTTON2, INPUT_PULLUP ) ;                  // Input for control button 2

 //Serial.println("started vs mjk Reached"); 
 SendNextionTextBoxCommand("page2.t0.txt=","Initialize VS1053");  
...
# if defined ( USETFT )
  ...
  NextionBufferingUpdate.attach ( 0.5, UpdateNextionBuffer ) ;                    // Every 500 msec

  
  dbgprint ( "Selected network: %-25s", ini_block.ssid.c_str() ) ;
  SendNextionTextBoxCommand("page2.t0.txt=","Connecting To: "+ String(ini_block.ssid)); 
  
  //Serial.println("Reached fddassfs"); 
  ...
  SendNextionTextBoxCommand("page2.t0.txt=","Successfully Connected,Streaming Started"); 
  SendNextionCommand("page2.tm0.en=0");
  SendNextionCommand("page 1");
  SendNextionCommand("page0.h0.val="+String(vs1053player.getVolume()));
  }
  else
...
void loop()
{
  ...
  if ( ini_block.newpreset != currentpreset )          // New station or next from playlist requested?
  {
    ...
    else
    {
      ...
      SendNextionTextBoxCommand("page0.g0.txt=","Connecting To "+ String(host));
      SendNextionTextBoxCommand("page0.t0.txt=","New preset Req: " + String(currentpreset));
      SendNextionTextBoxCommand("page0.g1.txt=","");
      SendNextionTextBoxCommand("page0.t2.txt=","");
 ...
 //sprintf ( reply1, "ringbuf %d, stream %d",rcount, mp3client->available()) ;
 //SendNextionTextBoxCommand("page1.t5.txt=","Buff:"+String(rcount));
  ...
  if ( datamode == HEADER )                            // Handle next byte of MP3 header
  {
    ...
    else if ( b == '\n' )                              // Linefeed ?
    {
      ...
      if ( chkhdrline ( metaline.c_str() ) )           // Reasonable input?
      {
		...
		{
			icyGenre = metaline.substring(10) ;            // Get GenreName
...
char* analyzeCmd ( const char* par, const char* val )
{
  ...
  if ( argument.indexOf ( "volume" ) >= 0 )           // Volume setting?
  {
    ...
    SendNextionCommand("page0.h0.val="+String(ini_block.reqvol));
  ...
  else if ( argument == "getnetworks" )               // List all WiFi networks?
  {
   ...
   else if ( argument == "list")               // List all Presets
  {
    getpresetsforNextion();
    //sprintf ( reply, presetlist.c_str() ) ;    // 
  }
  else
  {
